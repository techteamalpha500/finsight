#!/usr/bin/env python3
"""
Individual Deployment Script for import-stocks Lambda
This script handles deployment of only the import-stocks Lambda function
"""
...
def build_import_stocks_package():
    ...
    if requirements_file.exists():
        ...
        critical_deps = ['openpyxl', 'boto3', 'requests']
    ...
    with zipfile.ZipFile(zip_file, 'r') as zipf:
        ...
        critical_files = ['index.py', 'requirements.txt']
        ...
        # Check for openpyxl
        openpyxl_found = any('openpyxl' in f for f in files)
        if openpyxl_found:
            print("✅ openpyxl found in ZIP")
        else:
            print("❌ openpyxl not found in ZIP")
    
    print(f"✅ import-stocks package built successfully: {zip_file}")
    return zip_file

def deploy_terraform():
    """Deploy only the import-stocks Lambda using Terraform"""
    print("🚀 Deploying import-stocks Lambda with Terraform...")
    ...
    run_command(["terraform", "plan", "-target=aws_lambda_function.import_stocks"])
    
    # Apply deployment
    print("🚀 Applying Terraform deployment...")
    run_command(["terraform", "apply", "-target=aws_lambda_function.import_stocks", "-auto-approve"])
    
    print("✅ import-stocks Lambda deployed successfully!")

def main():
    """Main deployment function"""
    print("🚀 Starting import-stocks Lambda deployment...")
    print("=" * 60)
    ...
    build_import_stocks_package()
    
    # Deploy with Terraform
    deploy_terraform()
    
    print("=" * 60)
    print("🎉 import-stocks Lambda deployment completed successfully!")
    print("📊 You can now test the import-stocks functionality")
